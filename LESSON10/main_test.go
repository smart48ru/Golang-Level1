...
//Bench Test
func BenchmarkValue_Division(b *testing.B) {
	var (
		testTable = []struct {
			name string
			args *Value
	...
	for _, testCase := range testTable {
		b.Run(testCase.name, func(t *testing.B) {
			if AreaCalculated.Division(testCase.args) != testCase.want {
				b.Errorf("Benche_Division() = %v, want %v", AreaCalculated.Addition(testCase.args), testCase.want)
			}
		})
	}
}
//Testify test
func Test_Testefy_Value_Square(t *testing.T) {
	var (
		testTable = []struct {
			name string
			args *Value
	...
	for _, testCase := range testTable {
		//t.Logf("Square() = %v, want %v", AreaCalculated.Square(testCase.args), math.Sqrt(testCase.args.A))
		result := AreaCalculated.Square(testCase.args)
		assert.Equal(t, math.Sqrt(testCase.args.A), result,
			"Square() = %v, want %v", AreaCalculated.Square(testCase.args), math.Sqrt(testCase.args.A))
	}
}

func TestTestefyValue_Division(t *testing.T) {
	var (
		testTable = []struct {
			name string
			args *Value
	...
	for _, testCase := range testTable {
		result := AreaCalculated.Division(testCase.args)
		assert.Equal(t, testCase.want, result,
			"Square() = %v, want %v", AreaCalculated.Division(testCase.args), testCase.want)
	}
}
